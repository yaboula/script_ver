...
RegisterNetEvent('qb-inventory:server:SetStashItems', function(stashId, items)
	local src = source
	if type(stashId) ~= 'string' or stashId == '' then return end
	if type(items) ~= 'table' then return end
	if not Stashes[stashId] or Stashes[stashId].isOpen ~= src then return end
...
RegisterNetEvent('inventory:server:addTrunkItems', function(plate, items)
	...
	if type(items) ~= 'table' then return end
	if not Trunks[plate] or Trunks[plate].isOpen ~= src then return end
...
RegisterNetEvent('inventory:server:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
	...
	if not toSlot or toSlot < 1 or toSlot > Config.MaxInventorySlots then return end

	if type(itemName) ~= 'string' or not QBCore.Shared.Items[itemName:lower()] then return end
	if type(itemCosts) ~= 'table' or not next(itemCosts) then return end

	local recipe
	for _, craftItem in pairs(Config.CraftingItems or {}) do
		if craftItem and craftItem.name == itemName then
			recipe = craftItem
			break
		end
	end
	if not recipe or type(recipe.costs) ~= 'table' then return end

	for costName, costAmount in pairs(recipe.costs) do
		if itemCosts[costName] ~= costAmount then return end
	end
	for clientCostName in pairs(itemCosts) do
		if recipe.costs[clientCostName] == nil then return end
	end

	for k, v in pairs(recipe.costs) do
		local need = (tonumber(v) or 0) * amount
		if need <= 0 then return end
		local current = Player.Functions.GetItemByName(k)
		if not current or (current.amount or 0) < need then
			return
		end
	end
	...
	local repGain = (tonumber(recipe.points) or tonumber(points) or 0) * amount
	if repGain < 0 then repGain = 0 end
	Player.Functions.SetMetaData("craftingrep", Player.PlayerData.metadata["craftingrep"] + repGain)
...
RegisterNetEvent('inventory:server:CraftAttachment', function(itemName, itemCosts, amount, toSlot, points)
	...
	local recipe
	for _, craftItem in pairs((Config.AttachmentCrafting and Config.AttachmentCrafting.items) or {}) do
		if craftItem and craftItem.name == itemName then
			recipe = craftItem
			break
		end
	...
	local repGain = (tonumber(recipe.points) or tonumber(points) or 0) * amount
	if repGain < 0 then repGain = 0 end
	Player.Functions.SetMetaData("attachmentcraftingrep", Player.PlayerData.metadata["attachmentcraftingrep"] + repGain)
	TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
end)
