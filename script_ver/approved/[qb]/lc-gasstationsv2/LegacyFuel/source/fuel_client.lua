...
RegisterNUICallback('pay', function(data, cb)
	...
	if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),GetEntityCoords(vehicle)) < 5 then
		...
		TriggerServerEvent('LegacyFuel:pagamento01',math.floor(new_perc),false,VehToNet(vehicle),math.floor(new_perc),pumpLocation)
...
RegisterNUICallback('notifytext', function(data, cb)
	...
	TriggerEvent("gas_station:Notify","info",text)
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if not pumpObject then
			...
			if GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * (Config.MaxJerryCanAmmo/45) >= 0 then

				currentFuel = oldFuel + fuelToAdd

				SetPedAmmo(ped, 883325847, math.floor(GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * (Config.MaxJerryCanAmmo/45)))

			else

				TriggerEvent("Notify","info",Config.Strings.JerryCanEmpty)
...
AddEventHandler(eventName, function(ped, vehicle)
	...
	while isFueling do
		...
		DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Config.Strings.CancelFuelingJerryCan .. "\n"..Config.Strings.JerryCan..": ~g~" .. Round(GetAmmoInPedWeapon(ped, 883325847) / Config.MaxJerryCanAmmo * 100, 1) .. "%~w~ | "..Config.Strings.Vehicle..": ~g~" .. Round(currentFuel, 1) .. "%~w~")
