...
AddEventHandler("LegacyFuel:pagamento01",function(price,isJerryCan,vehicle,fuel,key)
	...
	if key then
		...
		query = Utils.Database.fetchAll(sql, {['@gas_station_id'] = key});
		...
		if query and query[1] then
			...
			Utils.Database.execute(sql, {['@gas_station_id'] = key});
	...
	if price > 0 then
		...
		if isJerryCan or output.stock > amount then

			local account = "bank"

			if Utils.Framework.tryRemoveAccountMoney(source,price,account) then

				if isJerryCan then

					Utils.Framework.insertWeaponInInventory(source,"weapon_petrolcan",1,{ammo = 1000})

					TriggerClientEvent("gas_station:Notify",source,"success","Paid <b>$"..price.." </b> for the <b>jerry can</b>.",8000)
					...
					if key and query and query[1] then
						...
						Utils.Database.execute(sql, {['@gas_station_id'] = key, ['@stock'] = ((output.stock) - amount), ['@price'] = price, ['@amount'] = amount});
						...
						Utils.Database.execute(sql, {['@gas_station_id'] = key, ['@income'] = 0, ['@title'] = "Fuel sold ("..amount.." Liters)", ['@amount'] = price, ['@date'] = os.time()});

					end

					TriggerClientEvent('fuel:setvehiclefuel',source,vehicle,fuel)

					TriggerClientEvent("gas_station:Notify",source,"success","Paid <b>$"..price.." </b> in "..amount.." liters.",8000)

				end

			else

				TriggerClientEvent("gas_station:Notify",source,"error","Insuficient money.",8000)

			end

		else

			TriggerClientEvent("gas_station:Notify",source,"error","Insuficient fuel stock (Max: "..output.stock.." liters).",8000)
...
AddEventHandler('LegacyFuel:LixeiroCB01', function(key)
	...
	if key then
		...
		local query = Utils.Database.fetchAll(sql, {['@gas_station_id'] = key});
...
AddEventHandler('LegacyFuel:LixeiroCB02', function()
	...
	if Utils.Framework.tryRemoveAccountMoney(source,Config.RefuelWithJerryCanCost,account) then

		TriggerClientEvent("gas_station:Notify",source,"success","Paid $"..Config.RefuelWithJerryCanCost,8000)
