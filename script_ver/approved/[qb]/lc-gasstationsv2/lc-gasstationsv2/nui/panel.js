...
let domSinksHardened = false;

function hardenDomSinks() {
    if (domSinksHardened) {
        return;
    }
    ...
    const wrapStringSanitizer = (methodName) => {
        const original = $.fn[methodName];
        if (typeof original !== "function") return;

        $.fn[methodName] = function (...args) {
            if (typeof args[0] === "string") {
                args[0] = window.Utils.sanitizeHtml(args[0]);
            }
            return original.apply(this, args);
        };
    };

    wrapStringSanitizer("html");
    wrapStringSanitizer("append");

    domSinksHardened = true;
}
...
function renderFuelTypeTab(gas_station_business, dark_theme) {
    ...
    Object.keys(fuel_type_list).forEach(fuelType => {
        ...
        // Fallback limits in case the config is missing or incomplete
        const defaultMin = 0;
        const defaultMax = 10;

        const priceLimits = config?.gas_station_types?.price_limits?.[fuelType] || { min: defaultMin, max: defaultMax };
			...
			<div class="control-box" id="control-box-${fuelType}">
				...
				<div class="slider-container">
					...
					<input type="range" id="slider-${fuelType}" class="slider" 
						min="` + (priceLimits.min * 100) + `" 
                        max="` + (priceLimits.max * 100) + `" 
    ...
    Object.keys(electric_fuel_type_list).forEach(fuelType => {
        ...
        let image = sanitizePathSegment(electric_fuel_type_list[fuelType].image);

        // Fallback limits if config or price limits are missing
        const defaultMin = 0;
        const defaultMax = 10;

        const priceLimits = config?.gas_station_types?.price_limits?.electric || { min: defaultMin, max: defaultMax };
...
function openDialogStartContract(type,ressuply_id,amount,relationship_upgrade) {
    ...
    for (const fuelType of fuelTypes) {
        const basePrice = gas_station_ressuply[fuelType] * amount;
        if (type == 1) {
            const discount = Math.floor(basePrice * discountPercent);
            finalPrices[fuelType] = basePrice - discount;
        } else {
            finalPrices[fuelType] = basePrice;
        }
    }
