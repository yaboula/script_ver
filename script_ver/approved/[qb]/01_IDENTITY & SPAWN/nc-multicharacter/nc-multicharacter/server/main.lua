...
RegisterNetEvent('nc-multicharacter:server:loadUserData', function(cData)
    local src = source
    -- [AUDIT AUD-002] Validación de cData
...
RegisterNetEvent('nc-multicharacter:server:createCharacter', function(data)
    local src = source
    -- [AUDIT AUD-002] Validación de data
    if type(data) ~= 'table' or type(data.cid) ~= 'number' then return end
...
RegisterNetEvent('nc-multicharacter:server:deleteCharacter', function(citizenid)
    local src = source
    -- [AUDIT AUD-002] Validación de string
...
-- [AUDIT AUD-002] Callback nc-multicharacter:server:GetServerLogs ELIMINADO: exponía logs sin autenticación
