...
Utils.sanitizePathSegment = function (value) {
    if (value === null || value === undefined) {
        return "";
    }
...
Utils.sanitizeHtml = function (input) {
    ...
    while (node) {
        Array.from(node.attributes).forEach((attr) => {
            ...
            if ((attrName === "src" || attrName === "href" || attrName === "xlink:href" || attrName === "action" || attrName === "formaction") && /^(javascript:|vbscript:|data:text\/html)/i.test(attrValue)) {
                node.removeAttribute(attr.name);
                return;
            }

            if (attrName === "style" && /(expression\s*\(|url\s*\(\s*[\"']?javascript:)/i.test(attrValue)) {
                node.removeAttribute(attr.name);
            }
        });
...
Utils.showCustomModal = function (config) {
    ...
    const dialogClassRaw = mergedConfig.dialogClass || "modal-dialog modal-dialog-centered";
    const dialogClass = String(dialogClassRaw).replace(/[^a-zA-Z0-9_\-\s]/g, "").trim() || "modal-dialog modal-dialog-centered";
    const modalTemplate = `
        <div id="confirmation-modal" class="modal fade">
    ...
    const safeBodyImage = Utils.sanitizeUrl(mergedConfig.bodyImage);
    if (safeBodyImage) {
        const $imageContainer = $("<div>", { class: "d-flex justify-content-center m-2" });
        const $image = $("<img>", { src: safeBodyImage, class: "w-50" });
    ...
    if (mergedConfig.bodyHtml) {
        const safeBodyHtml = Utils.sanitizeHtml(mergedConfig.bodyHtml);
        if (safeBodyHtml) {
            $modalBody.append(safeBodyHtml);
        }
    }
    ...
    mergedConfig.inputs.forEach(inputConfig => {
        ...
        if (inputConfig.type === "select") {
            ...
            const safeCustomHtml = Utils.sanitizeHtml(inputConfig.html || "");
            const $customInput = $(safeCustomHtml);
