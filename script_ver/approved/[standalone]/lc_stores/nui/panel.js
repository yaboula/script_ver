...
// Fallback to ensure Utils.post works even if lc_utils is unavailable or misloaded
(function ensurePostFallback(){
    try {
        if (!window.Utils) window.Utils = {};
        const getRes = () => {
            try { return (window.resourceName || (typeof GetParentResourceName === 'function' ? GetParentResourceName() : null)); }
            catch (_) { return window.resourceName || null; }
        };
        if (typeof window.Utils.post !== 'function') {
            window.Utils.post = function(event, data){
                const resName = getRes();
                if (!resName) {
                    console.error('[lc_stores] NUI post fallback could not determine resource name');
                    return Promise.resolve();
                }
                return fetch(`https://${resName}/post`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ event, data })
                }).catch(err => console.error('[lc_stores] NUI post fallback error:', err));
            };
            console.debug('[lc_stores] Applied NUI post fallback');
        }
    } catch (e) {
        console.error('[lc_stores] Failed to apply NUI post fallback:', e);
    }
})();
...
window.addEventListener("message", async function (event) {
	...
	if (item.price >= 0){
        ...
        // Ensure role is a number; default to 0 if missing
        role_level_id = parseInt(item.dados.role ?? 0, 10);
        try {
            const uiPerms = config?.roles_permissions?.ui_pages || {};
            console.debug('[lc_stores] Boss menu opened. role_level_id=', role_level_id, 'ui_pages_permissions=', JSON.stringify(uiPerms));
        } catch (e) {}
		...
		// Defensive: ensure store_categories is an array
		if (!Array.isArray(store_categories)) {
			store_categories = [];
		}
		...
		if (item.isMarket) {
			...
			*/
			...
			if(item.update != true){
				...
				if (firstPage) {
					...
					console.warn('[lc_stores] No available categories to display');
				}
			}
			...
			*/
			...
			if(item.update != true){
                ...
                const uiPagesPerms = (config && config.roles_permissions && config.roles_permissions.ui_pages) ? config.roles_permissions.ui_pages : {};
                for (const page in uiPagesPerms) {
                    if (role_level_id < uiPagesPerms[page]){
...
function openPage(pageN){
    if ((config?.roles_permissions?.ui_pages?.[pageN] == undefined) || role_level_id >= config.roles_permissions.ui_pages[pageN]){
        $(".pages").css("display", "none");
        ...
        // Provide clear feedback when a page click is blocked by permissions
        try {
            alert(Utils.translate("insufficient_permission"));
        } catch (e) {
            alert("You dont have permission to that");
        }
    }
}
