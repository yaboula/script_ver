...
local pending_open_request = false
...
function openOwnerUiCallback(market_id)
    current_market_id = market_id
    print("[lc_stores] E pressed on owner marker. market_id=", tostring(market_id))
    TriggerServerEvent("stores:getData",current_market_id)

    -- Fallback: if server doesn't respond quickly, open buy modal client-side
    pending_open_request = true
    SetTimeout(1500, function()
        if pending_open_request and current_market_id and not menu_active then
local market_cfg = Config.market_locations[current_market_id]
            if market_cfg then
local market_type = market_cfg.type
local type_cfg = Config.market_types[market_type]
                if type_cfg and type_cfg.categories then
local market_categories = {}
                    for _, cat in ipairs(type_cfg.categories) do
                        if Config.market_categories[cat] then
                            market_categories[cat] = Config.market_categories[cat]
                        end
                    end
                    print("[lc_stores] Fallback opening buy modal for", current_market_id, "price=", tostring(market_cfg.buy_price))
                    SendNUIMessage({
                        showmenu = true,
                        price = market_cfg.buy_price,
...
AddEventHandler('stores:open', function(dados,update,isMarket)
    print("[lc_stores] stores:open received. update=", tostring(update), "isMarket=", tostring(isMarket), "role=", tostring(dados and dados.role))
    -- Extra debug to diagnose missing role or permissions
local ok, encoded = pcall(function()
        return json.encode({ role = dados and dados.role, config_roles_permissions = dados and dados.config and dados.config.roles_permissions })
    end)
    if ok then
        print("[lc_stores] stores:open payload snapshot:", encoded)
    else
        print("[lc_stores] stores:open payload snapshot failed to encode")
    end
...
AddEventHandler('stores:openRequest', function(price, market_categories)
    print("[lc_stores] stores:openRequest received. price=", tostring(price), "categories=", type(market_categories))
    pending_open_request = false
    SendNUIMessage({
        showmenu = true,
        price = price,
...
RegisterNUICallback('post', function(body, cb)
	if cooldown == nil then
		...
		-- Debug: log incoming NUI events to help diagnose clicks doing nothing
		if body and body.event then
			print("[lc_stores] NUI post received:", tostring(body.event))
		end
...
local target_enabled = Utils.Config.custom_scripts_compatibility and Utils.Config.custom_scripts_compatibility.target ~= "disabled"
local target_available = Utils.Target and type(Utils.Target.createTargetInCoords) == "function"

    -- Try target interactions first if enabled and available
    if target_enabled and target_available then
local ok, err = pcall(function()
            createTargetsThread()
        end)
        if not ok then
            print("[lc_stores] Target creation failed, falling back to E markers:", err)
        end
    end

    -- Always create E-key markers as a safe fallback so interaction works even without target
    createMarkersThread()
