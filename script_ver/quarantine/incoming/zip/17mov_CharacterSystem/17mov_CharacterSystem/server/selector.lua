    ...
    MySQL.ready(function()
        ...
        if databaseName then
            local columnResults = MySQL.query.await(
                string.format("SELECT TABLE_NAME, COLUMN_NAME, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = \"%s\" AND DATA_TYPE = \"varchar\" AND COLUMN_NAME IN (?)", databaseName, 48),
