...
AddEventHandler("stores:buyUpgrade", function(market_id, data)
    ...
    Wrapper(_source, market_id, "buyUpgrade", true, function(user_id)
        ...
        local upgrade = Utils.Database.fetchAll("SELECT " .. data.id .. "_upgrade FROM `store_business` WHERE market_id = @market_id", {

            ["@market_id"] = market_id

        })

        local current_level = upgrade[1][data.id .. "_upgrade"]

        if current_level < 5 then

            local price = getConfigMarketType(market_id).upgrades[data.id].price

            if tryGetMarketMoney(market_id, price) then

                Utils.Database.execute("UPDATE `store_business` SET " .. data.id .. "_upgrade = " .. data.id .. "_upgrade + 1 WHERE market_id = @market_id", {

                    ["@market_id"] = market_id

                })

                insertBalanceHistory(market_id, 1, Utils.translate("upgrade_expenses"):format(Utils.translate(data.id .. "_upgrade")), price)
...
Utils.Callback.RegisterServerCallback("stores:loadBalanceHistory", function(source, cb, market_id, data)
    ...
    local balance = Utils.Database.fetchAll("SELECT * FROM `store_balance` WHERE market_id = @market_id AND id < @last_balance_id ORDER BY id DESC LIMIT 50", {
        ...
        ["@last_balance_id"] = data.last_balance_id
