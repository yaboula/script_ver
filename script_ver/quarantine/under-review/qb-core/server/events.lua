...
RegisterNetEvent('QBCore:Server:SetMetaData', function(meta, data)
    ...
    meta = meta:lower()
    if meta ~= 'hunger' and meta ~= 'thirst' and meta ~= 'stress' then return end
...
RegisterNetEvent('QBCore:Server:UseItem', function(item)
    local src = source
    if type(item) == 'table' and type(item.name) == 'string' and tonumber(item.amount) and tonumber(item.amount) > 0 then
