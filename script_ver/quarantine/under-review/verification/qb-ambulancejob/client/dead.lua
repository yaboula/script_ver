		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if deathTime > 0 then
                        -- DrawTxt(0.93, 1.44, 1.0, 1.0, 0.6, 'RESPAWN IN: ' .. deathtime .. ' SECONDS', {deathtime = math.ceil(deathTime)}, 255, 255, 255, 255)
                    else
                        -- DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, 'HOLD E FOR 5 SECONDS SECONDS TO RESPAWN FOR ' .. cost .. ' SECONDS', {deathtime = math.ceil(deathTime)}, 255, 255, 255, 255)
                    end                                       
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Config.MinimumRevive then
                    TriggerEvent('nc-deathscreen:die',LaststandTime)
                else
                    TriggerEvent('nc-deathscreen:die',LaststandTime)
