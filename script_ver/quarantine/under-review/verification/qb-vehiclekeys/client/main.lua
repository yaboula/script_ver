...
function HasKeys(plate)
    ...
    -- Normalize plate by removing spaces and converting to uppercase
    plate = string.gsub(plate, "%s+", ""):upper()
    return KeysList[plate] == true
end
    ...
    while true do
        ...
        if not HasKey and IsPedInAnyVehicle(PlayerPedId(), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), -1) == PlayerPedId() and QBCore ~= nil and not IsHotwiring then
            ...
            SetVehicleEngineOn(veh, false, false, true)
                ...
                if IsControlJustPressed(0, 74) then -- 74 is H key
...
function Hotwire(vehicle, plate)
    ...
    -- Trigger alarm and dispatch
    SetVehicleAlarm(vehicle, true)
    SetVehicleAlarmTimeLeft(vehicle, 10000) -- 10 seconds alarm
    exports['ps-dispatch']:VehicleTheft(vehicle)

    -- Start progress bar with random duration (10-15 seconds)
    local hotwireTime = math.random(10000,15000)
    QBCore.Functions.Progressbar("hotwiring_vehicle", "Attempting to hotwire...", hotwireTime, false, true, {
        ...
        -- 40% chance of success (adjust as needed)
        if math.random(1, 100) <= 40 then
            QBCore.Functions.Notify("Hotwire successful!", "success")
            HasKey = true
            ...
            -- 30% chance to break lockpick
            if math.random(1, 100) <= 30 then
                TriggerServerEvent('ps-inventory:server:BreakLockpick')
            end
        end
    end, function()
        -- Cancel case
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    print("[LOCKPICK] Attempting on vehicle:", plate, "Normalized:", normalizedPlate)

    -- Debug current keys before checking
    print("Pre-check KeysList:", json.encode(KeysList))
    
    if HasKeys(plate) then 
        QBCore.Functions.Notify('You already have keys for this vehicle', 'error')
        print("[DEBUG] Already has keys for:", normalizedPlate)
        return 
    end
    
    if #(pos - GetEntityCoords(vehicle)) > 2.5 then 
        QBCore.Functions.Notify('Too far from vehicle', 'error')
        return 
    end
    
    if GetVehicleDoorLockStatus(vehicle) <= 0 then 
        QCore.Functions.Notify('Vehicle already unlocked', 'error')
        return 
    end

    -- Start minigame
    local result = exports["j0-minigame"]:StartMinigame("shopLockpick", {
        difficulty = isAdvanced and "hard" or "medium",
        attempts = 3,
        time = 10
    })
    
    if result then
        print("[SUCCESS] Lockpick successful for:", normalizedPlate)
        ...
        Citizen.Wait(100)
        if not HasKeys(plate) then
            print("[WARNING] Keys not detected after assignment, forcing again")
            TriggerEvent('qb-vehiclekeys:client:AddKeys', plate)
        end
        ...
        -- If in driver seat, start engine
        if GetPedInVehicleSeat(vehicle, -1) == ped then
            SetVehicleEngineOn(vehicle, true, true, false)
        end
        
        QBCore.Functions.Notify('Vehicle unlocked and keys acquired!', 'success', 4500)
    else
        QBCore.Functions.Notify('Lockpick attempt failed!', 'error', 4500)
        if GetResourceState('ps-dispatch') == 'started' then
            exports['ps-dispatch']:CarJacking(vehicle)
        end
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
    end
    if math.random() <= (isAdvanced and Config.RemoveLockpickAdvanced or Config.RemoveLockpickNormal) then
        TriggerServerEvent('qb-vehiclekeys:server:breakLockpick', isAdvanced and 'advancedlockpick' or 'lockpick')
    end
end)
...
function LockpickDoor(isAdvanced)
    ...
    if #(pos - vehpos) > 2.0 then return end

    local vehLockStatus = GetVehicleDoorLockStatus(vehicle)
    if vehLockStatus <= 1 then -- Already unlocked
        QBCore.Functions.Notify("The door is already unlocked!", "error")
        return
    end

    -- Ensure all doors are closed and locked at start
    ...
    TaskPlayAnim(playerPed, "veh@break_in@0h@p_m_one@", "low_force_entry_ds", 3.0, 3.0, -1, 16, 0, 0, 0, 0)

    -- Trigger alarm
    SetVehicleAlarm(vehicle, true)
    SetVehicleAlarmTimeLeft(vehicle, 10000) -- 30 seconds alarm
    IsHotwiring = true

    -- Start j0-minigame
    local success = exports["j0-minigame"]:startMinigame("lockpick", {
        circles = isAdvanced and 4 or 3,
        time = isAdvanced and 6000 or 8000,
        complexity = isAdvanced and 3 or 2
    })
    ...
    if success then
        -- Unlock vehicle and open driver door
        SetVehicleDoorsLocked(vehicle, 1)
        SetVehicleDoorOpen(vehicle, 0, false, false)
        QBCore.Functions.Notify("Lockpick successful! Door is now open.", "success")
        
        -- Set player as owner if they weren't already
        if not HasKey then
            HasKey = true
            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
        end
    else
        -- Failed attempt - trigger dispatch
        exports['ps-dispatch']:VehicleTheft(vehicle)
        QBCore.Functions.Notify("Lockpick failed!", "error")
        
        -- Small chance to break the lockpick
...
function LockpickIgnition(isAdvanced)
    if not HasKey then 
        ...
        TaskPlayAnim(playerPed, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 3.0, 3.0, -1, 16, 0, 0, 0, 0)
        ...
        local options = {
            difficulty = isAdvanced and "hard" or "medium",
            attempts = isAdvanced and 3 or 2,
            time = isAdvanced and 10000 or 15000
        }

        local success = exports["j0-minigame"]:StartMinigame("shopLockpick", options)
        ...
        if success then
            ...
            if math.random(1, 100) < (isAdvanced and 20 or 40) then -- 20% for advanced, 40% for normal
                TriggerServerEvent('ps-inventory:server:BreakLockpick')
            end
            
            -- Small chance to damage vehicle
            if math.random(1, 100) < 15 then
                local currentHealth = GetVehicleEngineHealth(vehicle)
                SetVehicleEngineHealth(vehicle, currentHealth - 50.0)
            end
        end
    end
end
