...
RegisterNetEvent('qb-policejob:ToggleDuty', function()
    ...
    local dutymenu = {}
    print('Current duty status:', PlayerData.job.onduty)
...
CreateThread(function()
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            inImpound = true
            if PlayerJob.name == 'police' and PlayerJob.onduty and IsPedInAnyVehicle(PlayerPedId(), false) then
...
RegisterCommand('liverymenu', function()
    if not PlayerJob.name == 'police' then QBCore.Functions.Notify("You can't use this menu..", "error") return end
    local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), false) then	
