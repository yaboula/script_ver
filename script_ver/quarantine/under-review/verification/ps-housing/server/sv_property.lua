...
function Property:removeMloDoorsAccess(citizenid)
    ...
    for i = 1, self.propertyData.door_data.count do
        ...
        if door then
            ...
            ox_doorlock:editDoor(door.id, {characters = data})  -- Note: Typo fixed from original ('characters')
        end
    end
end
...
if property.propertyData.shell == 'mlo' then
    ...
    for i = 1, property.propertyData.door_data.count do
        ...
        if door then
            ox_doorlock:setDoorState(door.id, 0)  -- 0 means unlock the door
        else
            print(("^1[WARNING] Door ps_mloproperty%s_%s not found!"):format(property.property_id, i))
        end
    end
end
                end
