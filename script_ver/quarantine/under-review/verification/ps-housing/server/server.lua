CreateThread(function()
    Wait(2000) -- Wait 2 seconds to ensure resources are loaded
    
    local doorlockState = GetResourceState('ox_doorlock')
    
    if doorlockState ~= 'started' then
        -- If ox_doorlock is missing, throw an error (3 times for visibility)
        for i = 1, 3 do
            error('\n^1[ps-housing] CRITICAL ERROR: ox_doorlock resource is not running!\nThis resource REQUIRES ox_doorlock to function.^0')
            Wait(100)
        end
        return
    end
    
    -- Optional: Force DoorResource to 'ox' even if other checks exist later
    DoorResource = 'ox'
    
    print('^2[ps-housing] ox_doorlock confirmed, housing system ready.^0')
end)
...
if source and propertyData.shell == 'mlo' then
    -- Direct ox_doorlock call - no checks needed since we enforced it
    TriggerClientEvent("ps-housing:client:createOxDoors", source, {
        id = id,
        doors = propertyData.door_data
    })
    
    -- Optional: Verify door data before sending
    -- print(json.encode(propertyData.door_data, {indent=true}))
...
local function getMainDoor(propertyId, doorIndex, isShell)
    ...
    return DoorResource == 'ox' and exports.ox_doorlock:getDoorFromName(id) 
end
