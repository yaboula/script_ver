...
--         "key",
--         function()
--             -- Pass the property ID directly as a string
--             TriggerEvent("ps-housing:client:openManagePropertyAccessMenu", self.property_id)
--         end,
--         "ps-housing:client:openManagePropertyAccessMenu",
--         self.property_id  -- Pass the raw ID as the parameter
--     )
-- end
end
...
AddEventHandler("ps-housing:client:openManagePropertyAccessMenu", function(data)
    ...
    print("[DEBUG] Received data:", json.encode(data))

    -- Support multiple possible data structures
    local propertyId
    
    if type(data) == "string" then
        -- Handle case where just the ID is passed directly
        propertyId = data
    elseif data and data.propertyId then
        -- Handle { propertyId = "123" } format
        propertyId = data.propertyId
    elseif data and data.options and data.options.propertyId then
        -- Handle { options = { propertyId = "123" } } format
        propertyId = data.options.propertyId
    end

    if not propertyId then
        print("[PS-HOUSING] ERROR: Could not extract propertyId from data:", json.encode(data))
        return
    end

    local property = Property.Get(propertyId)
    if not property then
        print("[PS-HOUSING] ERROR: Property not found with ID:", propertyId)
        return
    end
