...
RegisterNetEvent("qb-gangmenu:server:withdrawMoney", function(amount)
	...
	TriggerEvent('qb-logs:server:createLog', 'gangmenu', 'Withdraw Money', xPlayer.PlayerData.charinfo.firstname .. ' ' .. xPlayer.PlayerData.charinfo.lastname .. ' successfully withdrew $' .. amount .. ' (' .. gang .. ')', false, src)
...
RegisterNetEvent("qb-gangmenu:server:depositMoney", function(amount)
	...
	MySQL.Async.execute('UPDATE gangmenu SET amount = ? WHERE job_name = ?', { GangaccountGangs[gang], gang })
	TriggerEvent('qb-logs:server:createLog', 'gangmenu', 'Deposit Money', xPlayer.PlayerData.charinfo.firstname .. ' ' .. xPlayer.PlayerData.charinfo.lastname .. ' successfully deposited $' .. amount .. ' (' .. gang .. ')', false, src)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			if Employee.Functions.SetGang("none", '0') then
				TriggerEvent('qb-logs:server:createLog', 'gangmenu', 'Gang Fire', Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.gang.name .. ")", false, src)
...
RegisterNetEvent('qb-gangmenu:server:HireMember', function(recruit)
	...
	if Player.PlayerData.gang.isboss == true then
		if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
			...
			TriggerEvent('qb-logs:server:createLog', 'gangmenu', 'Recruit', (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname).. ' successfully recruited ' .. Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false, src)
		end
	end
	TriggerClientEvent('qb-gangmenu:client:OpenMenu', src)
end)
